...
`ScopeAnchorRetriever` is constructed with trusted host context, then implements
LlamaIndex's normal `BaseRetriever` interface. The model-facing query is passed
to ScopeAnchor; resource filters are not part of the LlamaIndex query interface.
