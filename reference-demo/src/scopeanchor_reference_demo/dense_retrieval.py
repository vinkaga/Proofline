...
from scopeanchor import ScopeError, ScopeFilters, validate_scope_filter_fields
...
class QdrantDenseRetriever:
    ...
    async def search_tenant(
        ...
        if self._authorization is None:
            raise RuntimeError(
                ...
                "use search() through ScopeAnchor instead"
            )
    ...
    def search(
        ...
        *,
        ...
        """Implement ScopeAnchor's backend contract with Qdrant payload filters.
        ...
        if any(not values for values in filters.values()):
            return ()
        return self._search(query, _scopeanchor_filter(filters), limit)
...
def _scopeanchor_filter(filters: ScopeFilters) -> Filter | None:
    """Translate the supported ScopeAnchor allowlists into conjunctive Qdrant filters."""
