...
from scopeanchor_reference_demo.bounded_host import BoundedHostTrace, run_bounded_host
from scopeanchor_reference_demo.domain import Principal, RequestMode
from scopeanchor_reference_demo.evaluation_data import (
    EvaluationCaseSpec,
    EvaluationSuite,
    ExpectedOutcome,
)
...
def validate_release_evaluation(report: ReleaseEvaluationReport) -> None:
    ...
    failures = [
        f"{case.case_id}: {', '.join(case.violations)}" for case in report.cases if case.violations
    ]
    if failures:
...
async def _violations(
    ...
    if not set(case.required_resources).issubset({chunk.resource_id for chunk in returned_chunks}):
