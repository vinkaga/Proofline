...
from scopeanchor_reference_demo.domain import InteractionTrace, Principal, RequestMode
from scopeanchor_reference_demo.retrieval import RetrievalResult

_TRACER_NAME = "scopeanchor.reference_demo"
_configured_endpoint: str | None = None


@contextmanager
def trace_operation(name: str, attributes: dict[str, str | int | bool]) -> Iterator[trace.Span]:
...
def trace_tenant_retrieval(
    ...
    with trace_operation(
        ...
        {"scopeanchor.case_id": case_id, "enduser.id": principal.id},
    ) as span:
        if result.access_scope is None:
            span.set_attribute("scopeanchor.scope.resolved", False)
            raise ValueError("tenant retrieval traces require an access scope")
        span.set_attribute("scopeanchor.scope.resolved", True)
        span.set_attribute(
            "scopeanchor.scope.resource_count", len(result.access_scope.resource_ids)
        )
        span.set_attribute("scopeanchor.candidate_count", len(result.candidates))
