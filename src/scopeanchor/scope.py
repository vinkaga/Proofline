...
def _require_exact_keys(value: Mapping[str, object], *, name: str, keys: frozenset[str]) -> None:
    if set(value) != keys:
...
class RetrievalScope:
    ...
    def from_checkpoint(
        ...
        *,
        ...
        should normally use :meth:`scopeanchor.ScopedRetriever.resume`, which
        ...
        if _decode_string_mapping(encoded["binding"], name="binding") != _checkpoint_binding(
            binding
        ):
    ...
    def attenuate(
        ...
        return RetrievalScope(
            principal=self.principal,
            filters=(
                _FrozenFilters(updated_filters) if updated_filters is not None else self.filters
            ),
